...
export function DanglingLogo({ className = 'relative w-full h-full' }: { className?: string }) {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-[180px] sm:w-[230px] md:w-[280px] h-[110px] sm:h-[135px] md:h-[160px] flex flex-col items-center justify-center overflow-hidden select-none bg-transparent ${className}`}
    ...
    >
      ...
      />
      <span className="absolute bottom-0 text-[6px] sm:text-[8px] font-mono uppercase tracking-widest text-white/50 pointer-events-none select-none">
        DRAG LOGO TO SPIN
      </span>
    </div>
  );
